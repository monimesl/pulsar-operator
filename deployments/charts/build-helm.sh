...
DOCKER_IMAGE=monime/pulsar-operator:latest
if [[ -n $VERSION ]]; then
  HELM_VERSION="${VERSION//v/}" #helm version must be a SemVer
  DOCKER_IMAGE="monime/pulsar-operator:$VERSION"
...
OPERATOR_NAMESPACE=pulsar-operator
...
helm template pulsar-operator --include-crds --namespace $OPERATOR_NAMESPACE deployments/charts/operator/ >deployments/manifest.yaml
